...
import { approveBook, rejectBook } from '@/lib/actions/products'
...
export default function PendingProductsTable({ products }: { products: PendingBook[] }) {
                        ...
                        {filteredBooks.map((product) => (
                            <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-16 bg-gray-100 rounded overflow-hidden shrink-0 flex-col flex items-center justify-center">
                                            {product.image ? (
                                                <Image src={product.image} alt={product.title} width={48} height={64} className="w-full h-full object-cover" />
                                            ) : (
                                        ...
                                        <div>
                                            ...
                                            <div className="text-sm text-gray-500 line-clamp-1">{product.author}</div>
                                        </div>
                                    </div>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    {product.seller ? (
                                        <div>
                                            <div className="font-medium text-gray-900">{product.seller.fullName || 'Vendeur Anonyme'}</div>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <div className="font-medium text-gray-900">{product.price.toFixed(2)} MAD</div>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap">
