...
import { deleteBook, toggleBookStatus, bulkDeleteBooks, updateBookQuick, updateBookOrder, bulkUpdateBookPrices } from '@/lib/actions/products'
import { useRouter, usePathname } from 'next/navigation'
import ImageWithFallback from '@/components/ImageWithFallback'
import ProductCreativeModal from '@/components/admin/ProductCreativeModal'
import ProductDescriptionModal from '@/components/admin/ProductDescriptionModal'
...
export default function ProductsTable({ products, pageNumber, totalProviderPages, initialSearch, initialFilter = '', limit = 20 }: { products: Product[], pageNumber: number, totalProviderPages: number, initialSearch: string, initialFilter?: string, limit?: number }) {
    const [loading, setLoading] = useState<string | null>(null)
    const [selectedIds, setSelectedIds] = useState<string[]>([])
    const [bulkLoading, setBulkLoading] = useState(false)
    const [editingId, setEditingId] = useState<string | null>(null)
    const [editData, setEditData] = useState({ price: 0, stock: 0, active: true })
    const [creativeBook, setCreativeBook] = useState<Package | null>(null)
    const [creativeFormat, setCreativeFormat] = useState<'post' | 'story'>('post')
    const [descriptionBook, setDescriptionBook] = useState<Package | null>(null)
    const [descriptionFormat, setDescriptionFormat] = useState<'post' | 'story'>('post')
    const [n8nBook, setN8nBook] = useState<Package | null>(null)
    const [n8nFormat, setN8nFormat] = useState<'post' | 'story'>('post')
    const [previewCaptionBook, setPreviewCaptionBook] = useState<Package | null>(null)
    const [quickPublishing, setQuickPublishing] = useState<string | null>(null)
    const [toast, setToast] = useState<{ msg: string; ok: boolean } | null>(null)
    const [orderedBooks, setOrderedBooks] = useState(products)
    ...
    const handleQuickEdit = (product: Product) => {
        ...
        setEditData({ price: product.price, stock: product.stock, active: product.active })
    }

    // Publication immédiate sans modal
    const handleQuickPublish = async (product: Product) => {
        setQuickPublishing(product.id)
        try {
            ...
            if (data.success) {
                setToast({ msg: `✅ "${product.title}" envoyé à n8n pour publication`, ok: true })
    ...
    const handleSearch = (e: React.FormEvent) => {
        // Removed - handled by ProductFilters
    }

    const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.checked) {
            setSelectedIds(products.map(b => b.id))
        } else {
            setSelectedIds([])
        }
    }
    ...
    const handleExportExcel = async () => {
        try {
            ...
            const res = await fetch(`/api/admin/products/export-excel?${params.toString()}`)
    ...
    const handleExportJumia = async () => {
        if (selectedIds.length === 0) return
        try {
            const res = await fetch('/api/admin/products/export-jumia', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="p-4 bg-white border-b border-gray-200 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">
                        Total: <span className="text-gray-900 font-black">{products.length}</span> livres affichés
                    </p>
                            ...
                            <table className="w-full">
                                ...
                                <Droppable droppableId="products-list">
                                    {(provided) => (
                                        ...
                                        >
                                            {orderedBooks.map((product, index) => (
                                                                ...
                                                                <div className="flex items-center">
                                                                    ...
                                                                    <div className="ml-4">
                                                                        <div className="text-sm font-medium text-gray-900">
                                                                            {product.title}
                                                                            <span className={`px-1.5 py-0.5 text-[8px] font-black uppercase tracking-tighter rounded-md border ${product.isOriginal ? 'bg-amber-100 text-amber-700 border-amber-200' : 'bg-gray-100 text-gray-600 border-gray-200'}`}>
                                                                                {product.isOriginal ? 'Original' : 'Copie'}
                                                                            </span>
                                                                        </div>
                                                            ...
                                                            <td className="px-6 py-4 whitespace-nowrap">
                                                                {editingId === product.id ? (
                                                                    ...
                                                                    <button
                                                                        ...
                                                                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${product.active
                                                                ...
                                                                <div className="flex items-center justify-end gap-2">
                                                                    {editingId === product.id ? (
                                                                        ...
                                                                        <>
                                                                            ...
                                                                            <button
                                                                                onClick={() => setPreviewCaptionBook(product)}
                                                                            ...
                                                                            <button
                                                                                onClick={() => { setCreativeFormat('post'); setCreativeBook(product); }}
                                                                            ...
                                                                            <button
                                                                                onClick={() => { setCreativeFormat('story'); setCreativeBook(product); }}
                                                                            ...
                                                                            <button
                                                                                onClick={() => { setDescriptionFormat('post'); setDescriptionBook(product); }}
                                                                            ...
                                                                            <button
                                                                                onClick={() => { setDescriptionFormat('story'); setDescriptionBook(product); }}
                                                                            ...
                                                                            <button
                                                                                onClick={() => { setDescriptionFormat('post'); setDescriptionBook({...product, description: product.longDescription || product.description}); }}
                                                                            ...
                                                                            <button
                                                                                onClick={() => { setDescriptionFormat('story'); setDescriptionBook({...product, description: product.longDescription || product.description}); }}
                                                                            ...
                                                                            <button
                                                                                onClick={() => { setN8nFormat('post'); setN8nBook(product); }}
                                                                                className="text-green-600 hover:text-green-900 p-2 hover:bg-green-50 rounded transition-colors"
                                                                            ...
                                                                            <button
                                                                                onClick={() => handleQuickPublish(product)}
                                                                                disabled={quickPublishing === product.id}
                                                                            ...
                                                                            <Link
                                                                                href={`/admin/products/${product.id}/edit?${new URLSearchParams({ page: pageNumber.toString(), ...(initialSearch ? {search: initialSearch} : {}), ...(initialFilter ? {filter: initialFilter} : {}) }).toString()}`}
                                                                                className="text-blue-600 hover:text-blue-900 p-2 hover:bg-blue-50 rounded transition-colors"
                    ...
                    {viewMode === 'kanban' && (
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-10 gap-4 p-4">
                            {orderedBooks.map(product => (
                                <div key={product.id} className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow flex flex-col gap-3 relative group">
                                    <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10 bg-white/90 p-1 rounded-lg shadow-sm backdrop-blur-sm">
                                        <button onClick={() => handleToggleStatus(product.id, product.active)} className={`p-1.5 rounded-md ${product.active ? 'text-green-600 hover:bg-green-50' : 'text-gray-500 hover:bg-gray-100'}`} title={product.active ? 'Désactiver' : 'Activer'}>
                                            {product.active ? <Eye className="w-3.5 h-3.5" /> : <EyeOff className="w-3.5 h-3.5" />}
                                        </button>
                                        <Link href={`/admin/products/${product.id}/edit?${new URLSearchParams({ page: pageNumber.toString(), ...(initialSearch ? {search: initialSearch} : {}), ...(initialFilter ? {filter: initialFilter} : {}) }).toString()}`} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-md">
                                            <Edit className="w-3.5 h-3.5" />
                                        </Link>
                                        <button onClick={() => handleDelete(product.id, product.title)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-md">
                                            <Trash2 className="w-3.5 h-3.5" />
                                        </button>
                                    </div>
                                    <div className="aspect-[3/4] w-full overflow-hidden rounded-xl bg-gray-100">
                                        <img src={normalizeImage(product.image)} alt={product.title} className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).src = '/product-placeholder.png' }} />
                                    </div>
                                    <div className="flex-1 flex flex-col">
                                        <h3 className="font-bold text-sm text-gray-900 line-clamp-2 leading-tight">{product.title}</h3>
                                        <p className="text-xs text-gray-500 mt-1 truncate">{product.author}</p>
                                        <div className="mt-auto pt-3 flex items-center justify-between">
                                            <span className="font-black text-sm text-gray-900">{product.price} <span className="text-[10px] text-gray-500">MAD</span></span>
                                            <span className={`text-[10px] font-bold px-2 py-1 rounded-lg ${product.stock > 10 ? 'bg-green-100 text-green-700' : product.stock > 0 ? 'bg-orange-100 text-orange-700' : 'bg-red-100 text-red-700'}`}>
            ...
            <BulkPublishModal
                ...
                products={products.filter(b => selectedIds.includes(b.id))}
            />
            <CaptionPreviewModal
                isOpen={!!previewCaptionBook}
                onClose={() => setPreviewCaptionBook(null)}
                product={previewCaptionBook}
            />
        </div>
    )
}
