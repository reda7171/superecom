...
import { getBookById, updateBook, fetchAllAuthors, type BookInput } from '@/lib/actions/products'
...
export default function EditBookPage({ params, searchParams }: { params: Promise<{ id: string, locale: string }>, searchParams: Promise<{ page?: string, search?: string, filter?: string }> }) {
    ...
    const returnUrl = `/admin/products${returnUrlParams.toString() ? `?${returnUrlParams.toString()}` : ''}`

    const [id, setId] = useState<string>(productId)
    const [loading, setLoading] = useState(false)
    const [loadingData, setLoadingData] = useState(true)
    const [error, setError] = useState<string | null>(null)
    const [product, setBook] = useState<any>(null)
    ...
    async function loadBook(productId: string) {
        setLoadingData(true)
        try {
            const result = await getBookById(productId)
    ...
    const generateShortDescription = async () => {
        ...
        const titleInput = (document.getElementById('title') as HTMLInputElement)?.value || product?.title;
        const authorInput = (document.getElementById('author') as HTMLInputElement)?.value || product?.author;
        ...
        <div>
            ...
            <div className="mb-8">
                ...
                <p className="mt-2 text-sm text-gray-600">
                    Modifiez les informations du livre "{product.title}"
                </p>
            </div>
            ...
            {/* Form */}
            <form key={product.id} onSubmit={handleSubmit} className="bg-white rounded-lg border border-gray-200 p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select
                            id="status"
                            name="status"
                            defaultValue={product.status || 'APPROVED'}
                            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        >
                    ...
                    <div>
                        ...
                        <select
                            id="isBestSeller"
                            name="isBestSeller"
                            defaultValue={product.isBestSeller ? 'true' : 'false'}
                            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent font-medium"
                        >
                    ...
                    <div>
                        ...
                        <select
                            id="isOriginal"
                            name="isOriginal"
                            defaultValue={product.isOriginal ? 'true' : 'false'}
                            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent font-medium"
                        >
                    ...
                    <div className="md:col-span-2">
                        ...
                        <ImageInput defaultValue={product.image} bookTitle={product.title} />
                    </div>
