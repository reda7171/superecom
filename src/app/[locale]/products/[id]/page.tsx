import { getBookById, getBooks, getPopularBooks } from '@/lib/db/products'
...
export async function generateMetadata({
    ...
    if (!product) {
        return {
            title: 'Livre introuvable | SuperEcom',
        }
    }
    ...
    const keywords = [
        ...
        `acheter ${product.title} maroc`,
        `${product.category} maroc`,
        `livre ${product.category}`,
        `${product.author} livres`,
        'livres maroc',
        'librairie en ligne maroc',
        product.language === 'FR' ? 'livre français maroc' : product.language === 'AR' ? 'livre arabe maroc' : 'livre anglais maroc',
    ].filter(Boolean)

    return {
        title: `${product.title} - ${product.author} | SuperEcom`,
        description: `Achetez "${product.title}" de ${product.author} sur SuperEcom. ${product.description.slice(0, 120)}... Livraison rapide au Maroc. Prix: ${product.price} MAD. ${product.stock > 0 ? 'En stock' : 'Rupture de stock'}.`,
        keywords,
        openGraph: {
...
export default async function BookDetailPage({
    ...
    const isAdmin = user?.role === 'ADMIN' || user?.email === 'admin@superEcom.com'
    ...
    const [reviews, ratingData, categoryConfig, popularBooksForCrossSell, whatsappPhone, announcementEnabled, announcementMessage, announcementBgColor, announcementTextColor, relatedPosts] = await Promise.all([
        ...
        product.category ? getCategoryConfigByName(product.category) : Promise.resolve(null),
        getPopularBooks(6),
    ...
    const similarBooks = product.category
        ? await getBooks({ category: product.category, active: true })
        : []
    ...
    const frequentlyBoughtTogether = popularBooksForCrossSell
        .filter((b) => b.id !== product.id && !filteredSimilarBooks.some((sim) => sim.id === b.id))
        .slice(0, 2)
                                    ...
                                    {product.stock <= 5 && product.stock > 0 && (
                                        <div className="absolute top-6 right-6 rtl:left-6 rtl:right-auto z-20">
                                            <span className="px-5 py-2.5 bg-black text-white text-[9px] font-black rounded-full shadow-2xl animate-pulse uppercase tracking-widest">
                                                {tBook('OnlyLeft', { stock: product.stock })}
                                            </span>
                                        </div>
                                    )}
                                    {product.stock === 0 && (
                        ...
                        <div className="flex flex-col gap-12">
                            <div className="space-y-6">
                                <div className="flex flex-wrap items-center gap-4 mb-4">
                                    ...
                                    {isAdmin && (
                                        <a
                                            href={`/fr/admin/products/${product.id}/edit`}
                                ...
                                <h1 className="text-5xl md:text-7xl font-black text-black leading-none tracking-tighter">
                                    {product.title}<span className="text-gray-200">.</span>
                                </h1>

                                <p className="text-xl font-black text-gray-300 uppercase tracking-widest">
                                    {tCommon('By')} <Link href={`/authors/${encodeURIComponent(product.author)}`} className="hover:text-black transition-colors border-b-2 border-transparent hover:border-black">{product.author}</Link>
                                </p>
                            </div>
                            ...
                            <div className="relative group">
                                <div className="bg-white border border-gray-100 rounded-[3rem] p-10 shadow-2xl shadow-black/5">
                                    <div className="flex items-baseline gap-3 mb-4">
                                        <span className="text-7xl font-black text-black tracking-tighter leading-none">{product.price}</span>
                                ...
                                {product.stock > 0 && (
                                    <div className="space-y-4">
                                        ...
                                        {(product as any).previewUrl && (
                                            <a
                                                href={(product as any).previewUrl}
                                                target="_blank"
                                    ...
                                    <div className="flex flex-col gap-3 sm:gap-4">
                                        {frequentlyBoughtTogether.map(fbBook => (
                                            <div key={fbBook.id} className="flex items-center gap-3 sm:gap-4 bg-white p-2.5 sm:p-3 rounded-[1.5rem] shadow-sm hover:shadow-md transition-shadow">
                                                <Link href={`/products/${fbBook.id}`} target="_blank" rel="noopener noreferrer" className="flex gap-3 sm:gap-4 items-center flex-1 min-w-0 group">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="text-sm font-bold text-gray-400 uppercase tracking-widest leading-loose bg-white p-10 rounded-[2.5rem] border border-gray-50 italic space-y-4">
                                        ...
                                        {product.longDescription && (
                                            <p className="text-gray-600 font-medium normal-case tracking-normal mt-4">{product.longDescription}</p>
                                        )}
                                    </div>
                                </div>
                                    ...
                                    <div className="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-8">
                                        ...
                                        <dl className="space-y-5">
                                            <div className="flex justify-between items-center py-2 border-b border-gray-50">
                                                ...
                                                <dd className="text-[10px] font-black text-black tracking-widest">{product.isbn || 'SUPERECOM-GEN'}</dd>
                                            </div>
                                            <div className="flex justify-between items-center py-2 border-b border-gray-50">
                    ...
                    {filteredSimilarBooks.length > 0 && (
                        <section className="py-32 border-t border-gray-100">
                            <div className="flex items-center justify-between mb-20 px-2">
                                ...
                                <Link href="/products" className="text-[10px] font-black text-black uppercase tracking-widest bg-white px-8 py-4 rounded-full border border-gray-100 shadow-sm hover:shadow-xl transition-all">
                                    {tBook('ExploreAll')}
                                </Link>
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12">
                                {filteredSimilarBooks.map((similarBook) => (
                                    <ProductCard key={similarBook.id} {...similarBook} />
                                ))}
                            </div>
                        </section>
                    )}
