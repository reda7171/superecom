import { getBooks, getBookCategories, getBooksCount, getAllAuthors } from '@/lib/db/products'
...
export async function generateMetadata({ searchParams }: { searchParams: Promise<SearchParams> }): Promise<Metadata> {
    ...
    const title = categoryName ? `${categoryName} | SuperEcom` : String(tBooksPage('Title')) + ' | SuperEcom'

    return {
        title,
        description: `Explorez notre collection de livres ${categoryName ? `en ${categoryName}` : 'de développement personnel et business'} sur SuperEcom. Livraison disponible dans tout le Maroc.`,
    }
}
...
export default async function BooksPage({
    ...
    const [products, totalCount, categories, categoryConfig, authors, adsenseEnabled, adsensePublisherId, adsenseSlotBetweenBooks, popularPacks] = await Promise.all([
        getBooks(filterObj),
