...
export async function POST(req: Request) {
    try {
        ...
        const arabicBooks = books.filter(b => /[\u0600-\u06FF]/.test(b.title))
        
        if (arabicBooks.length > 0) {
            const prompt = `Transliterate these Arabic book titles into Latin characters (Transcription style used in Morocco/Maghreb, e.g., using numbers for some letters like 3, 7, 9 if common, or standard transcription). 
        ...
        for (const book of books) {
            ...
            const transliterated = transliterations[book.title] || book.title
            ...
            for (const col of columns) {
                ...
                switch (col) {
                    case 'Name':
                        val = hasArabic ? transliterated : book.title
                        break
                    case 'Name_FR':
                    case 'Name_EN':
                        val = hasArabic ? transliterated : book.title
                        break
                    case 'Name_AR':
                        val = hasArabic ? book.title : ''
                        break
                    case 'Description':
                    case 'Description_FR':
                    case 'short_description':
                    case 'short_description_FR':
                        val = book.longDescription || book.description
                        break
                    case 'SellerSKU':
                    case 'ParentSKU':
                        val = book.id.substring(0, 8).toUpperCase()
                        break
                    case 'Brand':
                        val = '1015941 - Books'
                        break
                    case 'PrimaryCategory':
                        val = '1000203 - Books, Movies and Music / Bestselling Books'
                        break
                    case 'GTIN_Barcode':
                    case 'isbn':
                        val = book.isbn || ''
                    ...
                    case 'author':
                        ...
                        break
                    case 'language':
                        val = book.language === 'ar' ? 'Arabic' : book.language === 'en' ? 'English' : 'French'
                        break
                    case 'book_type':
                        val = 'Paperback'
                        break
                    ...
                    case 'MainImage':
                        ...
                        if (val && !val.startsWith('http')) {
                            val = `${process.env.NEXT_PUBLIC_APP_URL || 'https://riwaya.ma'}${val.startsWith('/') ? '' : '/'}${val}`
                        }
                        break
                }
                rowData[col] = val
