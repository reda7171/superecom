...
export default function CosmosProducts({
  ...
  useEffect(() => {
    const orbit = products.filter(b => b.id !== centerBook.id).slice(0, 8)
    setOrbitOrder(orbit)
    anglesRef.current = orbit.map((_, i) => (i / orbit.length) * Math.PI * 2)
  }, [products, centerBook.id])

  // Auto-rotation du livre central (désactivée pendant le drag)
  useEffect(() => {
    if (!autoRotateCenter || dragging || products.length <= 1) return
    const interval = setInterval(() => {
      setCenterBook(prev => {
        const allIds = products.map(b => b.id)
  ...
  }, [autoRotateCenter, autoRotateInterval, products, dragging])
  ...
  const onPointerUp = () => {
    ...
    if (dropTarget !== null) {
      ...
      if (dropTarget === 'center') {
        ...
        setCenterBook(dragging.product)
        setOrbitOrder(prev => {
          const next = [...prev]
        ...
        setOrbitOrder(prev => {
          const next = [...prev]
          next[dropTarget] = dragging.product
          return next
        })
      } else {
  ...
  const handleClick = (product: OrbitBook) => {
    if (dragging) return
    ...
    >
      ...
      >
        ...
        {centerHovered && !dragging && (
          <div className="cosmos-product-tooltip">
      ...
      {bookPositions.map(({ x, y, size, bookIdx }) => {
        const product = orbitOrder[bookIdx]
        if (!product || failedImages.has(product.id)) return null
        ...
        return (
          <div
            ...
            onPointerDown={e => startDrag(e, bookIdx, product)}
            style={{
              position: 'absolute',
              left: `${relX}%`,
              top: `${relY}%`,
          ...
          >
            ...
            {isHovered && !dragging && (
              <div className="cosmos-product-tooltip">
                <p className="cosmos-tooltip-title">{product.title}</p>
                <p className="cosmos-tooltip-price">{product.price} MAD</p>
              </div>
            )}
          </div>
        )
      ...
      {dragging && (
        ...
        >
          {failedImages.has(dragging.product.id) ? (
             <div className="w-full h-full bg-gray-100 flex items-center justify-center p-2 text-center">
               <span className="text-[10px] font-black text-gray-400 line-clamp-3">{dragging.product.title}</span>
             </div>
          ) : (
            <img 
              src={normalizeImage(dragging.product.image)} 
              alt={dragging.product.title} 
              className="w-full h-full object-cover" 
            />
          )}
        </div>
