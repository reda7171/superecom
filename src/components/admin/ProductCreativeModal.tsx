...
export default function ProductCreativeModal({ isOpen, onClose, product, format = 'post' }: ProductCreativeModalProps) {
    ...
    const [editableTexts, setEditableTexts] = useState({
        badge: product?.category || 'Recommandation',
        title: product?.title || 'Titre',
        author: `par ${product?.author || 'Auteur'}`,
        price: product?.price?.toString() || '0',
        oldPrice: (product?.price ? Math.round(product.price * 1.3) : 0).toString(),
    ...
    useEffect(() => {
        if (!isOpen) {
            setTheme('dark') // reset
            setPositions({ badge: { x: 0, y: 0 }, logo: { x: 0, y: 0 }, product: { x: 0, y: 0 }, title: { x: 0, y: 0 }, author: { x: 0, y: 0 }, price: { x: 0, y: 0 }, delivery: { x: 0, y: 0 }, bg: { x: 0, y: 0 }, whatsapp: { x: 0, y: 0 } })
            setImageScale(1)
            setImageRotation(0)
            setLogoScale(3)
            ...
            setEditableTexts({
                badge: product.category || 'Recommandation',
                title: product.title,
                author: `par ${product.author}`,
                price: product.price.toString(),
                oldPrice: Math.round(product.price * 1.3).toString(),
                delivery_line1: 'Livraison',
                delivery_line2: 'Gratuite',
                whatsapp: '+212 600 00 00 00'
            })
            ...
            fetch(`/api/admin/products/${product.id}`).then(res => res.json()).then(data => {
                if (data.success) {
                    setFullBookData(data.product)
                    const baseDesc = useDescription === 'long' ? (data.product.longDescription || data.product.description) : data.product.description
                    const productUrl = `${window.location.origin}/fr/products/${data.product.id}`
                    const waPhone = whatsappPhone.replace(/[^0-9+]/g, '')
                    const waLink = `https://wa.me/${waPhone.startsWith('+') ? waPhone.slice(1) : waPhone}`
                    const hashtags = `#superEcom #lecture #livre #maroc #culture ${data.product.category ? `#${data.product.category.toLowerCase().replace(/\s+/g, '')}` : ''}`
                    setCaption(`${baseDesc}\n\n📖 Lien pour commander : ${productUrl}\n💰 Prix : ${data.product.price} MAD\n🚚 Paiement à la livraison + Livraison rapide !\n\n💬 Commander via WhatsApp : ${waLink}\n\n${hashtags}`)
                }
            }).catch(console.error)
        }
    }, [isOpen, product])
    ...
    useEffect(() => {
        if (fullBookData) {
            ...
            const productUrl = `${window.location.origin}/fr/products/${fullBookData.id}`
            const waPhone = editableTexts.whatsapp.replace(/[^0-9+]/g, '')
            const waLink = `https://wa.me/${waPhone.startsWith('+') ? waPhone.slice(1) : waPhone}`
            const hashtags = `#superEcom #lecture #livre #maroc #culture ${fullBookData.category ? `#${fullBookData.category.toLowerCase().replace(/\s+/g, '')}` : ''}`
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            try {
                const keywords = ['SuperEcom', 'Livre', product.title, product.author, 'Lecture', 'Maroc']
                finalDataUrl = injectXMPToPNG(dataUrl, product.title, keywords.filter(Boolean))
            } catch (err) {
            ...
            link.download = `creative_${product.title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}.png`
    ...
    const handleShare = async (platform: 'facebook' | 'instagram') => {
        ...
        try {
            ...
            const file = new File([blob], `creative_${product.title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}.png`, { type: mime })
            ...
            if (navigator.canShare && navigator.canShare({ files: [file] })) {
                await navigator.share({
                    files: [file],
                    title: product.title,
                    text: `Découvrez notre livre : ${product.title} !`
                })
                return
            }
    ...
    const handleQuickPublish = async () => {
        ...
        try {
            ...
            const blob = new Blob([u8arr], { type: mime })
            const file = new File([blob], `creative_publish_${product.id}.png`, { type: mime })
    ...
    const handlePreview = async () => {
        ...
        try {
            ...
            const productUrl = `${baseUrl}/fr/products/${product.id}`
            const text = caption || product.title
            
            const tags = `#superEcom #lecture #livre #maroc #culture ${product.category ? `#${product.category.toLowerCase().replace(/\s+/g, '')}` : ''}`
            ...
            if (newWindow) {
                newWindow.document.write(`
                    <html>
                        <head>
                            <title>Aperçu Publication - ${product.title}</title>
            ...
            <div className="bg-white w-full h-full flex flex-col md:flex-row overflow-hidden shadow-2xl">
                ...
                <div className="flex-1 bg-gray-100 p-8 flex items-center justify-center overflow-auto">
                    ...
                    >
                        ...
                        {/* Center Content (Product Cover) */}
                        <div className={`flex-1 flex items-center justify-center px-12 ${format === 'story' ? 'py-12' : 'py-6'} relative z-30 pointer-events-none`}>
                            <div 
                                className={`relative w-full ${format === 'story' ? 'max-w-[220px]' : 'max-w-[200px]'} mx-auto perspective-1000 cursor-move pointer-events-auto ${dragging === 'product' ? '' : 'transition-transform active:scale-95'}`}
                                style={{ transform: `translate(${positions.product.x}px, ${positions.product.y}px)`, touchAction: 'none' }}
                                ...
                                <button
                                    onClick={async () => {
                                        ...
                                        try {
                                            ...
                                            const blob = new Blob([u8arr], { type: mime })
                                            const filename = `creative_${product.title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_${Date.now()}.png`
