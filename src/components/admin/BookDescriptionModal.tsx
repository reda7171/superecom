...
export default function BookDescriptionModal({ isOpen, onClose, book, format = 'post' }: BookDescriptionModalProps) {
    ...
    useEffect(() => {
        if (!isOpen) {
            ...
            setEditableTexts({
                title: book.title,
                description: book.description || 'Une histoire captivante...'
            })
        }
    }, [isOpen, book])
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            link.download = `description-${book.title.replace(/\s+/g, '-').toLowerCase()}-${format}.png`
            link.href = dataUrl
            link.click()
        } catch (error: any) {
        ...
        try {
            ...
            if (newWindow) {
                newWindow.document.write(`
                    <html>
                        <head>
                            <title>Aperçu Description - Riwaya</title>
                            <style>
    ...
    const handleQuickPublish = async () => {
        ...
        try {
            ...
            const file = new File([blob], `desc_publish_${book.id}.png`, { type: mime })
                    ...
                    >
                        ...
                        {showCover && (
                            <div 
                                ...
                                style={{ transform: `translate(${positions.book.x}px, ${positions.book.y}px) scale(${imageScale}) rotate(${imageRotation}deg)` }}
                                ...
                                <button
                                    onClick={async () => {
                                        ...
                                        try {
                                            ...
                                            const blob = new Blob([u8arr], { type: mime })
                                            const filename = `desc_${book.title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_${Date.now()}.png`
