...
export default function CommentsSection({ postId, comments, totalCount }: CommentsSectionProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (result.success) {
            setContent('')
            setAuthorName('')
            setMessage({ type: 'success', text: result.message || 'Commentaire envoyé !' })
        } else {
    ...
    return (
        <section className="mt-20 pt-20 border-t border-gray-100 max-w-2xl mx-auto">
            ...
            <form onSubmit={handleSubmit} className="mb-16 bg-pixio-cream/50 p-8 rounded-[2rem]">
                <h3 className="text-sm font-black uppercase tracking-widest text-black mb-6">Laisser un commentaire</h3>
            ...
            <div className="space-y-8">
                {comments.length === 0 ? (
                    <p className="text-center text-gray-400 font-medium text-sm italic">Aucun commentaire pour le moment.</p>
                ) : (
