...
import { getBooks as getBooksDb, ProductFilters, getAllAuthors } from '@/lib/db/products'
...
export async function createBook(input: BookInput): Promise<BookResult> {
    try {
        ...
        await createAuditLog({
            ...
            entity: 'BOOK',
            entityId: product.id,
            details: `Livre créé: ${product.title} par ${product.author}`
        })
 ...
 */
export async function fetchBooks(filters: ProductFilters) {
    try {
        const products = await getBooksDb(filters)
...
export async function deleteAllBooks(): Promise<BookResult> {
    try {
        ...
        return { success: true, productId: 'all' }
    } catch (error: any) {
